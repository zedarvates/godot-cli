...
| Template registry | `template registry inspect` checks catalog integrity; `template validate` evaluates strict templates and optionally their bounded exact-version dependency closure against common/family schemas | Godot consumer readiness still requires explicit compatibility evidence for the checked scope. Decimal/exponent spec canonicalization, instantiation and migration remain separate gates. |
